...
/* 
    start experiments with a haking functions
*/
...
export async function main(ns) {
    ...
    ns.tprintf("worker script required %.2fGb", memoryTreshold);
    ...
    ns.tprintf("botnet %d memory %dGb max threads %d, used memory %dGb usage %.2f%%", 
    ...
    if (botnet.length) {
	ns.tprintf("botnet list:");
	botnet
	    .sort(function(a, b){ a.workers - b.workers})
	    .forEach(server => {
	    ns.tprintf("\t%s %dGb / %dGb, allow worker threads %d", 
    ...
    ns.tprintf("hacking servers %d/%d", scripts.size, servers.length);
    if (servers.length) {
	ns.tprintf("server list:");
	const table = new TableFormatter(ns,
	    ["    Name", "Chance", "Min ", "Cur",  "Avail",  "Max",    "Ratio", "Hack",   "Grow",   "Weak",   "Hacking", "Hack r", "Grow r", "Hack Th", "Hack $", "Grow Th", "Grow $", "Weak Th", "Sec Down", "max op th", "Ram req"],
	    ["    %s ",  "%.2f%%", "%.2f", "%.2f", "%.2f%s", "%.2f%s", "%.5f",  "%.2f%s", "%.2f%s", "%.2f%s", "%s", 	 "%.8f",   "%d",     "%d",      "%.2f%s", "%d",      "%.2f%s", "%d",      "%.2f",     "%d"       , "%dGb"]
	);
	...
	ns.tprintf("for optimal hack require max single server size %dGb, total size %dGb", oneServerRam, allServersRam);
