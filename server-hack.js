...
async function hackServer(lg, target, once, analyze, port) {
    ...
    while (true) {
        ...
        lg.lg(2, "%s analyze grow/hack on max threads %d", target, t);

        const availMoney = ns.getServerMoneyAvailable(target);
        lg.lg(2, "%s previous action %d, avail money availMoney %f, last %f", target, server.hackAction, availMoney, server.availMoney.value);
        ...
        if (server.preferAction == actionWeak && wt > 0 || server.analyzeChance < 0.01) {
            lg.lg(2, "%s prefer weak, chances %.2f", target, 100*server.analyzeChance);
        ...
        lg.ld(1, "gr %f >= gmr %f || gr == 0 => near empty", gr, gmr);
        lg.ld(1, "a %f == m %f || (gr < 1.01 && gr > 1.00) %f => full ", a, m, gr)
        lg.ld(1, "a %f >= m/gmr %f && a > gpa %f && a - gpa %f > m - gpa %f => near full", a, m/gmr, gpa, a-gpa, m-gpa);

        if (gr >= gmr || gr == 0) {
            lg.lg(2, "%s server near or empty", target);
            ...
            lg.lg(2, "%s server full, a == m", target);
        ...
        // more complex a is more then grow amount to max, and hack anount will be more then grow amount
            ...
            lg.lg(2, "%s server near full", target);
            ...
            lg.lg(2, "%s server has a few money, grow and hack", target); // why not try to max?
...
export async function main(ns) {
    ...
    if (!ns.serverExists(server)) {
        lg.lg(1, "server %s do not exists", server);
        return;
    }

    await hackServer(lg, server, runOnce, analyzeOnly, outputToPort);

    if (runOnce) lg.lg(1, "server-hack done target %s", server);
